...
$data = json_decode(file_get_contents("http://g-log.co/ForI5/?table=glog_marketplace_query_records&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO glogdb_clone.glog_marketplace_query_records (id,from,to,description,id_query)VALUES(".$id[$i].",".$from[$i].",".$to[$i].",'".$description[$i]."',".$id_query[$i].");",$conn);
...
{
	$deletedsql = "deleted FROM glog_marketplace_query_records WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `glog_marketplace_query_records`;";
