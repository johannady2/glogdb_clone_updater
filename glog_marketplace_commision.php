...
$data = json_decode(file_get_contents("http://g-log.co/ForI5/?table=glog_marketplace_commision&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO glogdb_clone.glog_marketplace_commision (id,commision,customer_id,customer_name)VALUES(".$id[$i].",".$commision[$i].",".$customer_id[$i].",'".$customer_name[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM glog_marketplace_commision WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `glog_marketplace_commision`;";
